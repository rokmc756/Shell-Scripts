...
class InstallVMs():
    ...
    def create_image( self, PREFIX, WORK_DIR, META_DATA, MEMORY, CPUS, TARGET_IMAGE, OSVAR, BASE_IMAGE, HOST_OS, PROD, CMD_BASE_DIR, IMG_BASE_DIR, IMG_FILE_NAME ):
        ...
        # VIP = IMG_BASE_DIR + PREFIX + IDENT + PREFIX + ".vmx"

        # Need to trim line added by this function.

        for line in fileinput.input( IMG_BASE_DIR.replace("\ "," ") + PREFIX + IDENT + PREFIX + ".vmx" ):
...
#                sys.stdout.write(line)
        ...
        fp = open( IMG_BASE_DIR.replace("\ "," ") + PREFIX + IDENT + PREFIX + ".vmx", 'a' )
    ...
    def wait_for_complete(self, PREFIX, HOST_OS, PROD, CMD_BASE_DIR, IMG_BASE_DIR, IMG_FILE_NAME ):
        ...
        global VIF
        while True:
            # print "sudo " + CMD_BASE_DIR + "vmrun -T ws getGuestIPAddress " + IMG_BASE_DIR + PREFIX + "/" + PREFIX + ".vmx"
            OUTPUT = os.popen("sudo " + CMD_BASE_DIR + "vmrun -T fusion getGuestIPAddress " + IMG_BASE_DIR + PREFIX + "/" + PREFIX + ".vmx")
